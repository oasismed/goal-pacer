...
- Scripts: `${CLAUDE_SKILL_DIR}/scripts/` (a pasta desta skill, que é o `app/` da instalação: um symlink para `<raiz>/app/`, com `<raiz>` padrão `~/.goal-pacer`; a instalação pode estar em outra pasta, e os scripts acham a raiz sozinhos). Rode sempre com caminho absoluto: `python3 ${CLAUDE_SKILL_DIR}/scripts/<script>.py ...`. Em desenvolvimento, rode a partir do clone: `python3 <repo>/scripts/<script>.py`. No Windows não há `python3`: use o caminho do campo `python3` de `<raiz>/jobs/instalacao.json` (padrão `~/.goal-pacer/jobs/instalacao.json`) no lugar dele.
- Pasta de dados: `<raiz>/dados/` (padrão `~/.goal-pacer/dados/`; `GP_DATA_DIR` sobrepõe; `status.py --doctor` mostra a pasta em uso). Nunca em Desktop, Downloads ou Documents.
...
Comandos (`S = python3 ${CLAUDE_SKILL_DIR}/scripts/onboarding.py`):
